...
- **探索和利用得掰扯。** 一直走已知最优，就永远发现不了更好的路；老去试新的，分又拿不到。
- **你给什么分，它就优化什么。** 这是最好玩也最吓人的一点。OpenAI 那个划船游戏的例子很出名
  ...
  奖励是沿途的分数道具，结果 agent 发现原地转圈刷道具比跑完比赛得分更高，于是它就一直转圈、
  一路撞船，还赢了分数。今天 RLHF 里的 reward hacking，和它是同一件事。
- **今天的 LLM agent 也吃这一套。** 「测试过了就算修好了」当奖励，它就可能去改测试。

所以你看到「agent」这个词的时候，最好先问一句对方在说哪一种。这个领域跑得很快，
定义也一直有争议——有人觉得只有会自己决定流程的才算，有人把带工具的一次调用也叫 agent。
下面讲的是现在最常见的那一种。
...
agent 是拿延迟和成本，换对开放式任务的适应能力。那篇文章的建议是先找最简单的方案，真需要了再加复杂度。可以先问自己几个问题：

1. **步骤能不能事先列出来？** 能列出来，就写成 workflow。
2. **有没有验证信号？** 测试能不能跑、答案能不能对、订单能不能查。没有验证信号，agent 做没做对，你很难知道。
3. **做错一步代价多大？** 发邮件、付钱、删数据这种不可逆的动作，要么不交给 agent，要么必须有人确认。

## ReAct：想一步，做一步

ReAct（Yao 等，2022）把推理和行动交替着来：模型先写下这一步在想什么，再调一个工具，看到结果接着想下一步。今天大多数 agent 的循环，都是它的变体。
