...
不知道从哪篇开始，可以沿着一条从数据到评估的主线走；已经知道问题在哪时，直接跳到对应那一格。

```mermaid
flowchart TB
    A["1 · 数据模块与契约<br/>先保证实验可复现"] --> B["2 · 正负样本设计<br/>说清监督信号的语义"]
    B --> C["3 · 负样本池与分布式 loss<br/>确认模型实际在比较什么"]
    C --> D["4 · 双塔与多意图表示<br/>决定检索模型的形状"]
    D --> E["5 · 离线 / 线上偏差<br/>检查评估是否覆盖真实分布"]
    E --> F["6 · 完整系统设计<br/>串起 signal、model、serving、evaluation"]
```

每篇都从一个故障现象出发，最后落到可以检查、证伪或上线前确认的判断标准。
