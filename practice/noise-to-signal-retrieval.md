...
| Candidate/用户塔 encoder | [Qwen3-Embedding-sub-1B](https://huggingface.co/Qwen/Qwen3-Embedding-sub-1B) | 检索质量、吞吐、长文本、向量大小与训练成本 | 4B/8B 可以作为容量上界，但会显著增加训练、准实时 编码与迭代成本 |
...
- **sub-1B**：足以从通用 embedding 初始化，同时允许频繁重训；相较更大模型，也更适合 准实时 刷新 用户 representation；
...
一个实用起点是：所有塔从同一个 Qwen3-Embedding-sub-1B checkpoint 初始化，candidate 与 用户 两侧使用独立参数或 adapter；profile/history 在 用户 侧共享主干，再使用不同 instruction、adapter 或 projection head。这样既保留共同语义空间，也允许不同输入学习不同的压缩方式。
