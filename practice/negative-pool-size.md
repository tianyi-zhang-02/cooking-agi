...
对比学习说白了就是让模型做选择题。而在多卡训练里，很容易出现这么一件事：你以为自己在出 上千选一 的题，实际上每张卡都只在做 B 选 1——**而且不会有任何报错**。选项数量与组成共同决定题目有多难；池子大，不等于负样本就一定好。
...
- 上千选一：随机采样碰到近邻干扰项的概率更高，模型通常需要画出更细的边界；但如果 上千个负样本仍然都很容易，大池子也只是更贵的送分题。
...
结果就可能是：你有 N 张卡，每张卡的批大小是 B，你心里默认负样本池是 N×B。实际上每张卡只拿自己那 B 个样本互相当干扰项，题目是 B 选 1，不是 上千选一。
