...
几乎所有这类设计文档都会写「未观测候选不能当负样本」。然后在训练那一节写「使用批内负样本」。

**这两句话是矛盾的。** 批内负样本就是把批次里其他样本的候选当成当前样本的负样本，而那些候选正是随机抽来的未观测内容。

这个矛盾不用回避——批内负样本提供的规模是真实的，去掉它对比学习就没法训。但它需要**显式修正**，因为它的采样分布不是均匀的：一个内容出现在批次里的概率正比于它在数据里的频率，所以**热门内容被当成负样本的次数远多于长尾内容**，模型会系统性地压低热门内容的分数。

标准做法是 logQ correction，把打分减去采样概率的对数：

$$s'(u,c) = s(u,c) - \log Q(c)$$

其中 $Q(c)$ 是 $c$ 出现在批次中的估计概率，用流式频率估计就够。
...
3. 批内负样本有没有做采样修正？没有的话，我实际在优化的目标是什么？
