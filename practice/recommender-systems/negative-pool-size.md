...
对比学习说白了就是让模型做选择题。而在多卡训练里，很容易出现这么一件事：你以为自己在出上千选一的题，实际上每张卡都只在做几十选一——**而且不会有任何报错**。选项数量与组成共同决定题目有多难；池子大，不等于负样本就一定好。
...
- 上千选 1：随机采样碰到近邻干扰项的概率更高，模型通常需要画出更细的边界；但如果那上千个负样本仍然都很容易，大池子也只是更贵的送分题。
...
于是可能出现这个局面：你有 $N$ 张卡、每张卡的批大小是 $B$，心里默认负样本池是 $N \times B$。实际上每张卡只拿自己那 $B$ 个样本互相当干扰项，题目一直是 $B$ 选 1。$N$ 是几十的时候，这中间差一个数量级以上。
