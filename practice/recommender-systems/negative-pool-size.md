...
对比学习说白了就是让模型做选择题。而在多卡训练里，很容易出现这么一件事：你以为自己在出 1024 选 1 的题，实际上每张卡都只在做 64 选 1——**而且不会有任何报错**。选项数量与组成共同决定题目有多难；池子大，不等于负样本就一定好。
...
- 1024 选 1：随机采样碰到近邻干扰项的概率更高，模型通常需要画出更细的边界；但如果那 1023 个负样本仍然都很容易，大池子也只是更贵的送分题。
...
举个例子：假设有 16 张卡、每张卡的批大小是 64，你心里默认负样本池是 16×64=1024。实际上每张卡只拿自己那 64 个样本互相当干扰项，题目仍然是 64 选 1。
