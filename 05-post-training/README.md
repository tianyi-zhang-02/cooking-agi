...
2. *SFT：模仿能到哪儿，到哪儿为止* —— 待写。交叉熵为什么对个别 token 不敏感，以及为什么"逼模型给答案"会强化幻觉
3. *偏好从哪来* —— 待写。奖励模型怎么训、Bradley-Terry、偏好标签本身的质量问题
...
6. *可验证奖励：当奖励不需要学* —— 待写。为什么它顺带收窄了 reward hacking，以及它的边界

**三、代价与现实**

7. *对齐税：变听话之后失去了什么* —— 待写。多样性下降、熵坍塌、泛化与多样性的权衡
8. *后训练的基础设施* —— 待写。采样吞吐、训练-推理数值一致、长会话的上下文管理。**这一层最少被讨论，而收益常常在这里**
9. [同一个基座能走多远](same-base-different-posttraining.md) —— 一次难得的自然实验：基座锁死，只改后训练

标斜体的是待写。**这份清单不是路线图作秀，它就是待办事项**——写完一篇就把斜体去掉。
