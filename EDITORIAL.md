...
站点以中文作为默认阅读面，但不把英文术语藏起来：

- 中文概念卡是正面，专业名词通过 glossary 显示标准英文原词；
- 每张概念卡必须有一一对应的完整英文背面，不能只放一句摘要；
- 中英文在当前卡片原地切换，不能要求读者回到页首或跳到另一页才做对照；
- 公式、shape、例子和限制条件必须在两面保持等价；
- 独立英文页继续保留，适合整篇英文阅读，但不是双语对照的主要入口。

新增或改写概念时，中英文两面必须在同一次更新中完成。无法准确翻译时，宁可先不发布
这张卡，也不要留下长期漂移的两套解释。

## 图表规范

图必须表达仅靠文字不容易快速看清的关系。每张图只回答一个视觉问题，使用短标签，并保持统一语法：圆角节点表示结果，圆柱表示版本化状态或证据，菱形表示 gate，实线箭头表示正向路径，虚线箭头表示反馈或控制。只有在能改善阅读顺序时才用分组；发布前必须实际 render 每个 Mermaid block。
