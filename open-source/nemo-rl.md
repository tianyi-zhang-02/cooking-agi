# NVIDIA NeMo-RL：我在别人的训练框架里做了什么

**中文** · [English](nemo-rl.en.md) · [返回开源项目](README.md)

> 阅读时间：约 12 分钟 · 类型：贡献笔记 · 时效性：Evolving · 最近审阅：2026-08

NeMo-RL 是 NVIDIA 开源的 LLM 后训练框架，管的是预训练之后的那一段：SFT、RLHF、GRPO、蒸馏。

我在里面做的事，说白了就是读代码，然后找两种东西。一种是代码在做一件它根本不需要做的事，另一种是代码没在做它自己声称在做的事。听起来很朴素，但这两类问题在一个快速迭代的训练框架里非常多。

下面按由易到难排。第一类不需要任何强化学习背景，第四类你得知道训练器和推理引擎是怎么配合的——那部分背景我放到需要它的地方再讲。
...
数据集配置里写错一个键，会被静默丢掉。你以为设置生效了，其实那行字从头到尾没人读过。

另一个数据集的 `subset` 参数更彻底，文档里写着、配置里接受、代码里从来不用。查这个的时候顺手发现文档描述的验证集划分也是错的。

选「最佳 checkpoint」的时候，如果两个指标打平，选哪一个取决于遍历顺序。同样的实验跑两次可能拿到不同的模型。

最后一个我很喜欢：有一句写得很清楚的报错，是死代码。函数在解析目标之前，就去读一个只有解析成功才会被写入的字典键。所以真出错的时候，程序死于一个裸 `KeyError`，那句为这个情况精心准备的提示，永远轮不到它出场。
...
关键只有一句话：log-softmax 的归一化常数 `log Z` 对每一项都是同一个数，在 top-k 的比值里会抵消掉。既然会抵消，那次全词表的归一化就是白算的，直接在取出来的六十四列上做就行。

有意思的是，认出这个形状之后，就会到处看见它：

| 场景 | 实际算了 | 真正需要 |
| --- | --- | --- |
| 求某个 token 的概率 | 整个 softmax 分布 | 一个位置的值 |
| 蒸馏路径升精度 | 把 `[B, S, 15万]` 整个升到 fp32，再取 64 列 | 只对那 64 列升精度 |
| 跨词表蒸馏的投影 | 投影到完整教师词表的 12.8 万列 | 只保留其中 8192 列 |
...
唯一能破坏这个恒等式的，是在完整词表上做一次重新归一化。代码里恰好没有这一步，那里的归一化只发生在留下来的 8192 列内部。
...
然后它确实算出了一个收窄后的 mask。接着它只把概率值返回出去，把那个 mask 扔了。五个调用点，全都还在用原来那个没收窄的 mask。

这为什么不是小事？因为被「mask 掉」的位置，填进去的值是 `0.0`。而这里是对数概率，`log p = 0` 意味着 `p = 1`。这不是「忽略这个位置」，这是「百分之百确定」。

同一个位置在采样时的真实概率是有限的，对数大约是负五点四。于是 `exp(5.4) ≈ 224` 这么一个凭空造出来的比值，就流进了训练和推理一致性的健康指标里。更糟的是在序列级重要性采样下，这个差值会沿着整条序列累加，再取指数，最后虚高的权重乘的是整条序列的损失。到这一步就不只是指标脏了，是真的进梯度。

最有说服力的旁证，其实是仓库自己写的。另一处代码给参考策略关掉了同款过滤，理由写着 *"-inf mismatches … cannot be resolved by masking"*。作者自己早就不信任这个机制了。

顺带说一个相邻的问题。六个 advantage estimator，有的返回张量，有的返回元组，还有的把指标挂在实例上让调用方用 `hasattr` 去捞。于是调用点写成了猜谜游戏。这不是六个函数各自有毛病，是缺一个契约，统一成一个数据类就解决了。
...
维护者开了个 issue：跨节点的权重同步目前只支持 vLLM，能不能也支持 SGLang。
...
结果是九点零四秒降到三点七七秒，模块数从七千一百五十降到五千二百五十六。
