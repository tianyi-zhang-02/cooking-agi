...
对我来说，理解底层不是多记几个系统名词，而是能顺着公式找到代码，再看清它实际怎样执行、哪一步可能出了偏差。

我开始给 NeMo RL 做贡献，并不是因为这些问题已经想明白了，恰恰是因为没有想明白。开源代码给了我一条很具体的学习路径：从一个可复现的问题出发，沿调用链找到实际使用这份数据或配置的位置，用数学或实验说明哪里不对，再让维护者和 CI 检查我的判断是否站得住。
...
自己读源码时，很容易在“我大概懂了”那里停下。开源贡献不允许这样：你得说明哪里有问题，补上能复现它的测试，讲清楚哪些场景还没验证，再接受熟悉这套系统的维护者追问。

因此，开源的意义不只是把代码免费放出来，也不只是累计合并数量。写成代码、测试和文档后，自己的理解才有机会被别人检查、使用和继续改进：

- 一个 bug fix 让当前问题消失；
- 一个 regression test 让同类问题以后更难回来；
- 一个清楚的接口或 invariant 让后来的人更容易扩展系统；
- 一次 review，可能帮你发现本地测试没覆盖的情况，让改动在其他任务里也能可靠运行。
...
给模型做 post-training，远不止调用一次 `loss.backward()`。
...
问题是 rollout 的时间并不整齐。只要某个环境响应慢，或者某条回答特别长，其他 GPU 就可能得等着。SingleController 的做法，是让生成和训练各自持续工作：
...
这样一来，生成和训练就能同时进行。但 trainer 拿到的数据，可能是旧版本 policy 生成的。提高吞吐的同时，还得检查数据落后了几个版本、怎样做 importance correction，以及出错后如何处理。
...
所以我现在读一条训练路径，不会停在“配置字段存在”或“函数被调用”。我会继续找到实际使用这个值的位置，确认它是否真的影响了后续计算和梯度。
...
同样，rollout pump 结束后，train pump 可能还在 drain queue。这时，即使主流程已经进入收尾，也不能停止检查 watchdog 是否报告了异常。可靠的异步系统至少要说清楚两件事：**训练数据来自哪个版本的 policy；系统卡住时，监控和异常处理是否还在工作。**
...
这段经历也解释了我为什么会转向 SingleController：做久以后，最有价值的已经不是再找到一个局部优化，而是开始理解一个完整子系统应该守住哪些 invariant。局部 correctness、数学等价性和分布式数据流原来不是三类互不相干的题，它们都在确保代码实际执行的，仍然是原本想做的实验。
...
对我来说，这才是从“修过一些 bug”到逐渐能负责一个子系统的过程。不必声称自己什么都懂，但要知道接下来该查哪里、怎样验证自己的判断，也要能坦白说“这一部分我还没测过”。
