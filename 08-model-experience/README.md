...
我说的 Model Experience，不只是某一次回答写得好不好，而是持续使用后的感受：它还记得前面的事吗？能听懂纠正吗？关键决定能由我来控制吗？用得久了，是否真的更省心？
...
助手 B 记得你的膝盖曾经受伤，但在使用这条信息前会确认它是否仍然有效；当计划不合适时，它能记住你的修改，而不是下次继续犯同样的错。
...
模型不该今天记住一件事，明天就完全忘掉；也不该只凭一次偶然行为，就认定你一直如此。

## 为什么单轮评估不够

有些问题得连续用上几轮，甚至几周，才会发现：
...
- [Personal AGI](../09-personal-agi/) 定义长期关系的目标。
- [表征与记忆](../02-memory/) 决定系统怎样保持连续性。
- [Search](../04-search/) 决定模型能提供怎样的信息空间。
- [Evaluation](../07-evaluation/) 把主观体验拆成可以验证的证据。
- [Human-in-the-Loop](../06-systems/human-in-the-loop.md) 让用户能确认关键操作、纠正错误，必要时交给人来处理。
